# <img alt="image" src="https://user-images.githubusercontent.com/74011724/195976879-4a1aa238-64bb-454e-9909-4250717aabae.png" width="40" height="40">  피크랩(pickRAP)
...
- 검색엔진에 최적화 되어있고 로딩 시간이 빠르며, 프레임워크 자체에서 지원하는 기능이 많아 `Next`를 사용합니다.
- 개발 경험 향상과 오류 방지를 위해 `Typescript`를 사용합니다.
- UI를 만들기 편하고 양질의 정보가 많으며 커뮤니티가 가장 커므로 `React`를 사용합니다. 

- Spring 설정 간편화 및 라이브러리 의존성 자동화가 가능한 `Spring Boot`를 사용하여 빠른 시간 안에 어플리케이션 완성이 가능하도록 합니다.
- 회원 관리에 대한 보안(인증과 인가, 권한)에 대한 처리를 해주기 위해 `Spring Security` 사용합니다.
- 서비스 사용에 이용되는 모든 데이터 정보를 관리하기 위해 `MySQL`을 사용합니다.

- 독립된 컨테이너로 동일한 환경 구축을 위해 `Docker`를 사용합니다.
- 배포 과정에서 안전하고 쉽게 클라우드 컴퓨팅 작업이 가능한 `AWS EC2`를 사용합니다.
- 마찬가지로 배포 과정에서 사용자의 데이터들을 클라우드에서 관리하기 위해 `AWS RDS`를 사용합니다.
- `Nginx`를 사용하여 웹서버와 WAS를 분리하고 reverse proxy로 보안을 고려합니다.
- API 자동 명세 및 테스트를 위한 `Swagger`를 사용할 예정입니다.

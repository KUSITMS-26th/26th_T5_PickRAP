...
- 원활한 통신 작업을 위해 Docker를 사용합니다.
- 배포 과정에서 안전하고 쉽게 클라우드 컴퓨팅 작업이 가능한 AWS EC2를 사용합니다.
- 마찬가지로 배포 과정에서 사용자의 데이터들을 클라우드에서 관리하기 위해 AWS RDS를 사용합니다.
- Nginx의 reverse proxy 기능을 사용하여 무중단 배포 환경을 구축합니다.
- API 자동 명세 및 테스트를 위한 Swagger를 사용할 예정입니다.

# 커밋 컨벤션

### github flow 전략

1. 작업( 기능/버그/리팩토링 ) 단위로 이슈 생성
2. `feat/#이슈넘버` 명명으로 브랜치 생성
3. feat → dev → main
